...
def mark_company_installment_paid(installment_id: int) -> Optional[Dict[str, Any]]:
    ...
    if installment.get("status") == STATUS_LUNAS:
        return recalculate_company_progress(int(installment["loan_company_id"])) or installment
...
def get_company_loan_detail_by_id(loan_company_id: int) -> Optional[Dict[str, Any]]:
    return get_company_loan_by_id(loan_company_id)
